...
\newpage
\section{最短路问题} % 包括斯坦纳树、最小环、负环、差分约束

\newpage
\section{生成树问题} % 包含矩阵树定理、拟阵
...
算法正确性证明如下：
\begin{enumerate}
	\item 必要性：在第二次DFS中，记反图DFS树的起点为 $T$，假设存在路径 $T\leadsto S$，则原图中存在路径 $S\leadsto T$ 且 $T$ 的标号大于 $S$ 的标号。考虑原图的DFS树，若 $T$ 是 $S$ 的祖先，则显然原图中 $T$ 可达 $S$。若 $T$ 不是 $S$ 的祖先，则由 $\operatorname{dfn}(T)<\operatorname{dfn}(S)$，$T$ 不在以 $S$ 为根的子树中，即 $T$ 所在的子树先访问到。
\end{enumerate}
...
\end{mdframed}
\subsubsection{SSP算法}
\subsubsection{原始对偶算法} % Primal-Dual
...
\part{概率论}
\section{期望问题}
\subsection{基本定义} % 包含随机变量和概率转化
\subsection{条件期望}
\subsection{经典模型} % 包含期望DP
...
\newpage
\part{字符串}
\section{前缀数组} % 包含KMP、失配树、最小循环子串等
...
\newpage
\section{线段树入门} % 包括猫树、划分树、吉司机、李超、zkw
...
\section{随机化算法}
\subsection{随机函数}
\subsection{模拟退火} % 包含爬山算法
\subsection{Moser算法}
\subsection{蒙特卡洛方法}

\newpage
\section{启发式算法}
\subsection{人类智慧} % 启发函数
\subsection{启发式搜索}
\subsection{启发式合并}
\subsection{启发式分裂}
...
\subsubsection{牛顿迭代}
\subsection{自适应辛普森}
\subsection{多项式插值} % 不用放入多项式章节

\newpage
\section{连续段问题}

\newpage
\section{编码问题} % 包含霍夫曼、括号序等
